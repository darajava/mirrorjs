...
let music;
let free = (document.free);

function onDeviceReady() {
  if (free) {
    admobid = {
      interstitial: 'ca-app-pub-4819202686048720/1429692479'
    };
...
let game = new Phaser.Game(Presets.width, Presets.height, Phaser.CANVAS, 'root', { preload: preload, create: create, update: update });
...
function preload() {
  ...
  game.load.audio('gobble', 'assets/gobble.mp3');
...
let games = [
  {
    name: 'same',
    description: 'Tap everything in common',
    weight: 0.6,
  },
  {
    name: 'different',
    description: 'Tap the difference',
    weight: 0.6,
  },
  {
    name: 'all',
    description: 'Tap everything',
    weight: 0.6,
  },
  {
    name: 'opponent',
    description: 'Tap the OTHER ',
    weight: 0.4,
  },
  {
    name: 'time',
    description: 'Tap after 10 seconds',
    weight: 0.7,
  },
  {
    name: 'patience',
  ...
  },
  {
    name: 'describe',
    descriptions: {
      sky: 'Tap everything in the sky',
      living: 'Tap all living things',
      manmade: 'Tap all manmade things',
      natural: 'Tap all natural things',
      animal: 'Tap all animals',
      person: 'Tap all people',
      fourlegs: 'Tap everything with 4 legs',
      twolegs: 'Tap everything with 2 legs',
    },
    weight: 0.8,
  },
  {
    name: 'fire',
    description: 'Tap like crazy!',
    weight: 0.4,
  },
  {
    name: 'drag',
    description: 'DRAG to the other side',
    weight: 0.4,
  },
  {
    name: 'coop',
    description: 'Put all chickens in the coop',
    weight: 0.4,
  },
  // {
  //   name: 'draw',
...
function addScoreSection() {
  ...
  graphics.drawRect(0, 0, Presets.width, dividerHeight );
  graphics.endFill();

  let graphics2 = game.add.graphics(0, 0);
  graphics2.lineStyle(10, 0x6CA7DA, 1);
  graphics2.beginFill(0x7CA7DA, 1);
  graphics2.drawRect(0, Presets.height - dividerHeight, Presets.width, dividerHeight );
  graphics2.endFill();

  // TODO: XXX: Make the score text use valign and center

  let style = { font: "bold 100px Arial" };
  let style2 = { font: "bold 40px Arial", fill: '#77a' };

  text2 = game.add.text(Presets.padding - 20, Presets.height/7 - dividerHeight/2 + 16, "0", style);
  text2.angle = 180;

  text2.x += Presets.width - Presets.padding - 20;
  text2.y += text2.height - 10;

  text1 = game.add.text(Presets.padding, Presets.height/7 * 6 - dividerHeight/2 + 16, "0", style);
  ...
  descText1.setTextBounds(0, Presets.height/2, Presets.width, Presets.height / 2);
  ...
  descText2.y = Presets.height - descText1.height + dividerHeight;
  descText2.angle = 180;
}
...
function placeCoopItemPlayer(item, player) {  
  item = {
    properties: {
      sky: false,
    },
  ...
  item.obj.events.onDragStop.add(onDragCoopStop, item);
...
function placeCoopItems(items) {
  ...
  // coop.obj.anchor.setTo(0.5, 0.5);

  coop.obj.x = Presets.width / 2 - coop.obj.width;
  coop.obj.y = Presets.height / 2 - coop.obj.height;
  coop.obj.scale.x = 2;
  coop.obj.scale.y = 2;
  ...
  for (let i = 0; i < numberOfItems; i++) {
    placeCoopItemPlayer(items[i], 1, items[i].properties[selectedGame.key])
  }

  for (let i = 0; i < numberOfItems; i++) {
    placeCoopItemPlayer(items[i], 2, items[i].properties[selectedGame.key]);
  }

  numberOfItems = oldNumberOfItems;
}
...
function placeItems() {
  ...
  switch (selectedGame.name) {
    ...
    case 'drag':
      ...
      break;
    case 'coop':
      placeCoopItems(thisRoundsItems);
      break;
  }
...
let correctSound, wrongSound, gobble;
...
let playerCoopDraggedCount = [0, 0];
function onDragCoopStop() {
  let onCoop = Phaser.Rectangle.intersects(this.obj.getBounds(), coop.obj);
  ...
  if (onCoop) {
    ...
    playerCoopDraggedCount[this.player - 1]++;
    if (playerCoopDraggedCount[this.player - 1] === numberOfItems + 1) {
      onWin(this);
      coop.obj.destroy();
      delete coop;
      playerCoopDraggedCount = [0, 0];
    }
  }
}
